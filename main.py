...
'''
@client.event
async def edit_message(texte):
    global message_bot
    try:
        msg = await channel.fetch_message(message_bot)
    ...
    for x in range(5):
        for y in range(8):
            if Grille[x][y] == Grille[x+1][y] == Grille[x+2][y] == Grille[x+3][y] in (1, 2):
                Grille[x][y], Grille[x+1][y], Grille[x+2][y], Grille[x+3][y] = 3, 3, 3, 3
                return "yes"
    for x in range(8):
        for y in range(5):
            if Grille[x][y] == Grille[x][y+1] == Grille[x][y+2] == Grille[x][y+3] in (1, 2):
                Grille[x][y], Grille[x][y+1], Grille[x][y+2], Grille[x][y+3] = 3, 3, 3, 3
                return "yes"
    for x in range(5):
        for y in range(5):
            if Grille[x][y] == Grille[x+1][y+1] == Grille[x+2][y+2] == Grille[x+3][y+3] in (1, 2):
                Grille[x][y], Grille[x+1][y+1], Grille[x+2][y+2], Grille[x+3][y+3] = 3, 3, 3, 3
                return "yes"
    for x in range(5):
        for y in range(5):
            if Grille[x][y] == Grille[x+1][y-1] == Grille[x+2][y-2] == Grille[x+3][y-3] in (1, 2):
...
#Todo: Changer couleur des joueurs
@client.event
                    ...
                    for i in range(8):
                        ...
                        for j in range(8):
                            if Grille[i][j] == 0:
                                ...
                                message_grille = message_grille + ":green_circle:|"
                    compteur += 1
